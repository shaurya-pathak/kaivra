...
Follow-up list for the architecture explainer, the Kaivra engine, and the voice-synced render pipeline.

## Completed Recently

- [x] Generate audio first and retime authored scene animations before render in the `voice_overlay` workflow.
- [x] Add a reusable scene retimer in `kaivra` so scene durations, focus timing, camera timing, and motion envelopes can scale with narration.
