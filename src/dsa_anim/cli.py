...
@click.option(
    "--voice-model",
    type=click.Path(exists=True),
    help="Sherpa VITS/Piper model (.onnx) or model directory. Falls back to DSA_ANIM_SHERPA_MODEL.",
)
@click.option(
    "--voice-tokens",
    type=click.Path(exists=True, dir_okay=False),
    help="Sherpa tokens.txt path. Defaults to the model directory or DSA_ANIM_SHERPA_TOKENS.",
)
@click.option(
    "--voice-data-dir",
    type=click.Path(exists=True, file_okay=False),
    help="Sherpa espeak-ng-data directory. Defaults to the model directory or DSA_ANIM_SHERPA_DATA_DIR.",
)
@click.option(
    "--voice-lexicon",
    type=click.Path(exists=True, dir_okay=False),
...
@click.option(
    ...
    type=int,
    default=None,
    help="Optional Sherpa speaker ID override. Falls back to DSA_ANIM_SHERPA_SPEAKER or 0.",
)
@click.option(
    "--voice-speed",
    type=float,
    default=None,
    help="Optional Sherpa speaking speed. Falls back to DSA_ANIM_SHERPA_SPEED or 1.0.",
)
@click.option(
    "--voice-pad",
    type=float,
    default=None,
    help="Silence padding to append after spoken narration per scene. Falls back to DSA_ANIM_SHERPA_PAD or 0.8 seconds.",
)
@click.option(
    "--voice-binary",
    default=None,
    help="Sherpa offline TTS executable name or absolute path. Defaults to DSA_ANIM_SHERPA_BIN or sherpa-onnx-offline-tts.",
)
@click.option(
    "--voice-artifacts-dir",
...
def theme_schema():
    ...
    from dsa_anim.themes.loader import theme_schema as build_theme_schema
...
def validate_theme(theme_file: str):
    ...
    from dsa_anim.themes.file_schema import load_theme_file
